...
Building on the multi-source #acr("SSL") model developed in Chapter 3, we propose a method that aggregates directional information from short audio segments recorded at different positions along a trajectory.
...
The task is formalized as a #acr("POMDP"), where the agent receives only multi-channel audio input and is rewarded based on the Word Error Rate (WER) achieved by a downstream ASR model.
...
The agent’s perception module reuses a convolutional feature extractor pretrained in a supervised setting on the single-source localization task introduced in Chapter 3, enabling it to ground its navigation policy in spatial auditory cues.
Observations are encoded as interaural spectral features (ILD and IPD), and the reward signal is provided by a pre-computed WER map evaluated on real ASR transcripts.

Building on this theoretical grounding, we introduce a novel sound-driven navigation task in which a robot must reposition itself in a reverberant environment to optimize #acr("ASR") performance.
The task is formalized as a partially observable #acr("MDP"), where the agent receives only multi-channel audio input and is rewarded based on the #acr("WER") achieved by a downstream #acr("ASR") model.
A full training pipeline is implemented using the PPO algorithm, with a custom neural architecture and an environment built on our simulator.
Observations are encoded as interaural spectral features (ILD and IPD), and the reward signal is provided by a pre-computed WER map evaluated on real ASR transcripts.
...
The contribution demonstrates how auditory perception can be cast as a reward optimization problem and illustrates the potential of DRL to produce interpretable, perception-driven navigation strategies.

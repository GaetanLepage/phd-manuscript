...
#let acrl(acr, plural: false, link: true) = {
  acros.display(acronyms => {
    if is-valid(acr) {
      ...
      if type(defs) == "string" {
        if plural {
          _display(acr, defs + "s", link: link)
        } else {
          _display(acr, defs, link: link)
        }
        ...
        if plural {
          if defs.len() == 1 {
            _display(acr, defs.at(0) + "s", link: link)
          } else if defs.len() == 2 {
            _display(acr, defs.at(1), link: link)
          } else {
            panic("Definitions should be arrays of one or two strings. Definition of " + acr + " is: " + type(defs))
          }
        } else {
          _display(acr, defs.at(0), link: link)
        }
      } else {
        panic("Definitions should be arrays of one or two strings. Definition of " + acr + " is: " + type(defs))
      }
...
#let acrf(acr, plural: false, link: true) = {
  if plural {
    _display(acr, [#acrlpl(acr) (#acr\s)], link: link)
  } else {
    _display(acr, [#acrl(acr) (#acr)], link: link)
  }
